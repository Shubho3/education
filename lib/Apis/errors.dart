...
class UnknownError extends TRKError {
  UnknownError() : super('Unknown Error occured.');
}

class TRKError implements Exception {
  TRKError(this.message);

  @override
  String toString() => 'Error: $message';

  final String message;
  factory TRKError.fromError([dynamic error]) {
    if (error is TRKError) {
      return error;
    }
    if (error is String) {
      return TRKError(error);
    }
    if (error is DioError) {
      if (error.error is TRKError) {
        return error.error;
      }
      return TRKError(error.message);
    }
    return UnknownError();
  }
}
...
MotionToast? showError(BuildContext context, Object e, {void Function()? onSubmit, String? title}) {
  ...
  return MotionToast.error(
    ...
    description: Text(TRKError.fromError(e).message),
