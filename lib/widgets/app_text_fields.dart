      ...
      children: [
        ...
        DecoratedBox(
          decoration: BoxDecoration(
            boxShadow: widget.shadow ?? [BoxShadow(color: Colors.black.withOpacity(0.05), offset: const Offset(0.0, 5.0), blurRadius: 10.sp)],
          ),
          child: TextFormField(
            ...
            style: TextStyle(color: widget.textColor ?? const Color(0xFF2D2D2D), fontWeight: FontWeight.w400, fontSize: 15.sp),
            ...
            decoration: InputDecoration(
              ...
              floatingLabelStyle: TextStyle(color: CC.blackPurple, fontSize: 14.sp),
              hintStyle: widget.hintStyle,
              fillColor: widget.fillColor ?? Colors.white,
              filled: true,
              contentPadding: EdgeInsets.fromLTRB(16.sp, 16.sp, 16.sp, 16.sp),
              labelText: widget.floatingLabel,
              labelStyle: TextStyle(color: CC.gray, fontSize: 14),
            ),
          ),
        ),
      ],
...
class AppDropdown<T> extends StatefulWidget {
  ...
  AppDropdown({
    ...
    if (items == null) {
      ...
      this.items = protoTypeItems!.map((e) => AppDropdownItem<T>(text: e, value: e as T)).toList();
    } else {
      this.items = items;
    }
  }
...
class _AppDropdownState<T> extends State<AppDropdown<T>> {
  ...
  Widget build(final BuildContext context) {
    ...
    return DecoratedBox(
      decoration: BoxDecoration(
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), offset: const Offset(0.0, 0.0), blurRadius: 4.0)],
      ),
...
class MultiSelectAppDropdown extends StatefulWidget {
  ...
  MultiSelectAppDropdown({
    ...
    if (items == null) {
      ...
      this.items = protoTypeItems!.map((e) => MultiSelectAppDropdownItem<String>(text: e, value: e)).toList();
    } else {
      this.items = items;
    }
  }
      ...
      child: PopupMenuButton(
        ...
        itemBuilder: (BuildContext context) => widget.items
            ...
            child: _theState.rebuild(
                  () => _SelectRow(
                onChange: (val) =>
                val ? onChanged([...selectedItem, item.value]) : onChanged(selectedItem.where((element) => element != item.value).toList()),
                selected: selectedItem.any((element) => element == item.value),
                text: item.text,
              ),
            ),
          ),
        ...
        child: AbsorbPointer(
          child: AppTextFormField(
            ...
            controller: TextEditingController(
              text: widget.items.where((item) => selectedItem.any((i) => i == item.value)).map((item) => item.text).join(', '),
            ),
            maxLines: 3,
            minLines: 1,
            readOnly: true,
